const css = `
/***************************
File generated by shrinker.ch
DateTime: 2022-05-13, 10:21:41
File list:
* general.css
* remote-arrow.css
* remote.css
* remote-channelSelector.css
* remote-funckey.css
* remote-slidekey.css
*****************************/
body{background-color:#222629;margin:0;padding:0;user-select:none}:root{--arrow-keys-box-size:200px;--arrow-border-size:1px;--arrow-border-color:#a8d0e6;--arrow-main-color:#222629;--lighter-main-color:#2c2f32;--active-color:#4a563a;--remote-list-height:70vh;--btn-side-length:calc(var(--arrow-keys-box-size)/2.5)}.arrow-keys-box{width:var(--arrow-keys-box-size);height:var(--arrow-keys-box-size);background-color:var(--arrow-main-color);position:relative;transform:rotate(45deg);border-radius:60px;border:calc(var(--arrow-border-size) * 1) var(--arrow-border-color) solid;overflow:hidden;margin:50px auto}.arrowBtn{width:calc(var(--arrow-keys-box-size)/2 - var(--arrow-border-size));height:calc(var(--arrow-keys-box-size)/2 - var(--arrow-border-size));position:absolute;border:var(--arrow-border-size) var(--arrow-border-color) solid}.arrow-up{left:0;top:0;border:0}.arrow-right{right:0;top:0;border-top:0;border-right:0}.arrow-left{left:0;bottom:0;border-left:none;border-bottom:0}.arrow-down{right:0;bottom:0;border:0}.arrow-icon{width:calc(var(--arrow-keys-box-size)/4);height:calc(var(--arrow-keys-box-size)/4);margin-top:calc(var(--arrow-keys-box-size)/8);margin-left:calc(var(--arrow-keys-box-size)/8);fill:var(--arrow-border-color)}.arrow-left>svg{transform:rotate(-135deg)}.arrow-up>svg{transform:rotate(-45deg)}.arrow-right>svg{transform:rotate(45deg)}.arrow-down>svg{transform:rotate(135deg)}.arrow-ok{width:var(--btn-side-length);height:var(--btn-side-length);background-color:var(--arrow-main-color);left:calc(50% - var(--btn-side-length)/2 - var(--arrow-border-size));top:calc(50% - var(--btn-side-length)/2 - var(--arrow-border-size));border:var(--arrow-border-color) var(--arrow-border-size) solid;border-radius:100px;z-index:99}.arrowBtn:active{background-color:var(--active-color)}@import url("./general.css");@import url("./remote-arrow.css");@import url("./remote-funckey.css");@import url("./remote-slidekey.css");@import url("./remote-channelSelector.css");#TVandBoxToggle{display:none}.toggleBtn{--toggleBtn-size:calc(var(--btn-side-length) * .85);width:var(--toggleBtn-size);height:var(--toggleBtn-size);border:var(--arrow-border-color) var(--arrow-border-size) solid;position:absolute;top:10px;right:10px;border-radius:100px;display:flex;justify-content:center;align-items:center}.toggleBtn>svg{fill:var(--arrow-border-color);width:32px;height:32px}.toggleBtn:active{background-color:var(--active-color)}.GoToSetTopBox{display:none}.TV-Btns{display:none}#TVandBoxToggle:checked ~ .toggleBtn{display:none}#TVandBoxToggle:checked ~ .SetTopBox-Btns{display:none}#TVandBoxToggle:checked ~ .GoToSetTopBox{display:flex}#TVandBoxToggle:checked ~ .TV-Btns{display:initial}#TVandBoxToggle:checked ~ .channel-selector{display:none}.footer{height:100px}@import url("./general.css");.channel-selector{z-index:99;background-color:var(--lighter-main-color);position:fixed;bottom:0;color:var(--arrow-border-color);border-radius:10px 10px 0 0;border:var(--arrow-border-color) var(--arrow-border-size) solid;padding:0 15px;height:var(--remote-list-height);transition:bottom .35s ease-in-out}.channel-activate-bar-box{width:100%;display:flex;justify-content:center}.channel-activate-bar{margin:0 auto;width:calc(100vw - 34px);padding:20px 0;display:flex;flex-direction:row;justify-content:space-between;border-bottom:var(--arrow-border-color) var(--arrow-border-size) solid}.channel-activate-bar-icon{width:20px;height:20px;margin-left:10px;margin-top:2px}.channel-activate-bar-title{height:20px;margin-left:10px}.channel-title-area{display:flex;flex-direction:row}.channel-chevron-area>svg{width:20px;height:20px;margin-right:10px;margin-top:2px}.channel-selecting-list{height:calc(100% - 75px);position:relative}.channel-selecting-list-inner{background-color:var(--arrow-main-color);border-radius:10px;margin:10px 0;padding:0 15px;height:100%;overflow:scroll;overflow-x:hidden}.channel-list-item{border-top:var(--arrow-border-color) var(--arrow-border-size) solid;padding:20px 0}.channel-list-item:active{width:calc(100%+40px);margin-left:-20px;padding-left:20px;border-radius:10px;border-top:1px solid rgba(0,0,0,1);background-color:var(--active-color)}.channel-list-item:active+.channel-list-item{border-top:1px solid rgba(0,0,0,1)}.channel-selecting-list-inner>div:first-child{border-top:0}.hide-list{bottom:calc(66px - var(--remote-list-height))}.bi-chevron-up{transform:rotate(0deg);transition:transform .4s ease-in-out}.rotate180{transform:rotate(180deg)}@import url("./general.css");.func-keys-box{display:flex;justify-content:space-around;flex-wrap:wrap;width:80%;margin:0 auto}.func-btns{color:var(--arrow-border-color);width:var(--btn-side-length);height:var(--btn-side-length);border:var(--arrow-border-color) var(--arrow-border-size) solid;border-radius:100px;margin:12px auto;display:flex;justify-content:center;align-items:center;position:relative}.func-fullname{font-size:14px;position:absolute;bottom:-20px}.func-btns>svg{fill:var(--arrow-border-color);width:36px;height:36px}.func-btns:active{background-color:var(--active-color)}@import url("./general.css");.slide-key-box{width:80%;margin:0 auto;display:flex;flex-direction:row;justify-content:space-around}.slide-key{border:var(--arrow-border-color) var(--arrow-border-size) solid;width:calc(50% - 10px);height:var(--btn-side-length);border-radius:50px;display:flex;flex-direction:row;overflow:hidden}.slide-key>div{color:var(--arrow-border-color);font-size:48px;font-weight:lighter;width:50%;display:flex;align-items:center;justify-content:center}.slide-key>div>svg{fill:var(--arrow-border-color);width:32px;height:32px}.slide-key>div:active{background-color:var(--active-color)}.left-slide-key{border-right:var(--arrow-border-color) var(--arrow-border-size) solid}.toggleBtn, .arrowBtn, .left-slide-key, .right-slide-key, .func-btns, svg {cursor: pointer;}.channel-selector-closer{width:100vw;height:100vh;position:absolute;display:fixed;left:0;top:0;z-index:98}.closer-hidden{display:none}
`

const bodyHTML = `
    <input type="checkBox" id="TVandBoxToggle">

    <div class="channel-selector-closer closer-hidden"></div>
    <div class="channel-selector hide-list">
        <div class="channel-activate-bar-box">
            <div class="channel-activate-bar">
                <div class="channel-title-area">
                    <div class="channel-activate-bar-icon">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" class="bi bi-collection" viewBox="0 0 16 16">
                            <path d="M2.5 3.5a.5.5 0 0 1 0-1h11a.5.5 0 0 1 0 1h-11zm2-2a.5.5 0 0 1 0-1h7a.5.5 0 0 1 0 1h-7zM0 13a1.5 1.5 0 0 0 1.5 1.5h13A1.5 1.5 0 0 0 16 13V6a1.5 1.5 0 0 0-1.5-1.5h-13A1.5 1.5 0 0 0 0 6v7zm1.5.5A.5.5 0 0 1 1 13V6a.5.5 0 0 1 .5-.5h13a.5.5 0 0 1 .5.5v7a.5.5 0 0 1-.5.5h-13z"/>
                          </svg>
                    </div>
                    <div class="channel-activate-bar-title">頻道選單</div>
                </div>

                <div class="channel-chevron-area">
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-chevron-up" viewBox="0 0 16 16">
                        <path fill-rule="evenodd" d="M7.646 4.646a.5.5 0 0 1 .708 0l6 6a.5.5 0 0 1-.708.708L8 5.707l-5.646 5.647a.5.5 0 0 1-.708-.708l6-6z"/>
                      </svg>
                </div>
            </div>
        </div>

        <div class="channel-selecting-list">
            <div class="channel-selecting-list-inner">
                <div class="channel-list-item" code="83" type="channel" onclick="sendCMDByObj(this)">83 無綫新聞台</div>
                <div class="channel-list-item" code="81" type="channel" onclick="sendCMDByObj(this)">81 翡翠台</div>
                <div class="channel-list-item" code="31" type="channel" onclick="sendCMDByObj(this)">31 數碼電視 31</div>
                <div class="channel-list-item" code="32" type="channel" onclick="sendCMDByObj(this)">32 數碼港台 32</div>
                <div class="channel-list-item" code="33" type="channel" onclick="sendCMDByObj(this)">33 中央電視台綜合頻道</div>
                <div class="channel-list-item" code="34" type="channel" onclick="sendCMDByObj(this)">34 中國環球電視網記錄頻道</div>
                <div class="channel-list-item" code="76" type="channel" onclick="sendCMDByObj(this)">76 HOY 國際財經台</div>
                <div class="channel-list-item" code="77" type="channel" onclick="sendCMDByObj(this)">77 HOY TV</div>
                <div class="channel-list-item" code="78" type="channel" onclick="sendCMDByObj(this)">78 HOY 資訊台</div>
                <div class="channel-list-item" code="82" type="channel" onclick="sendCMDByObj(this)">82 TVB Plus</div>
                <div class="channel-list-item" code="84" type="channel" onclick="sendCMDByObj(this)">84 明珠台</div>
                <div class="channel-list-item" code="85" type="channel" onclick="sendCMDByObj(this)">85 鳳凰衛視香港台</div>
                <div class="channel-list-item" code="96" type="channel" onclick="sendCMDByObj(this)">96 ViuTVsix</div>
                <div class="channel-list-item" code="99" type="channel" onclick="sendCMDByObj(this)">99 ViuTV</div>
            </div>
        </div>
    </div>


    <label class="toggleBtn GoToTV" for="TVandBoxToggle">
        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-hdd" viewBox="0 0 16 16">
            <path d="M4.5 11a.5.5 0 1 0 0-1 .5.5 0 0 0 0 1zM3 10.5a.5.5 0 1 1-1 0 .5.5 0 0 1 1 0z"/>
            <path d="M16 11a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2V9.51c0-.418.105-.83.305-1.197l2.472-4.531A1.5 1.5 0 0 1 4.094 3h7.812a1.5 1.5 0 0 1 1.317.782l2.472 4.53c.2.368.305.78.305 1.198V11zM3.655 4.26 1.592 8.043C1.724 8.014 1.86 8 2 8h12c.14 0 .276.014.408.042L12.345 4.26a.5.5 0 0 0-.439-.26H4.094a.5.5 0 0 0-.44.26zM1 10v1a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1v-1a1 1 0 0 0-1-1H2a1 1 0 0 0-1 1z"/>
          </svg>
    </label>

    <label class="toggleBtn GoToSetTopBox" for="TVandBoxToggle">
          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-tv" viewBox="0 0 16 16">
            <path d="M2.5 13.5A.5.5 0 0 1 3 13h10a.5.5 0 0 1 0 1H3a.5.5 0 0 1-.5-.5zM13.991 3l.024.001a1.46 1.46 0 0 1 .538.143.757.757 0 0 1 .302.254c.067.1.145.277.145.602v5.991l-.001.024a1.464 1.464 0 0 1-.143.538.758.758 0 0 1-.254.302c-.1.067-.277.145-.602.145H2.009l-.024-.001a1.464 1.464 0 0 1-.538-.143.758.758 0 0 1-.302-.254C1.078 10.502 1 10.325 1 10V4.009l.001-.024a1.46 1.46 0 0 1 .143-.538.758.758 0 0 1 .254-.302C1.498 3.078 1.675 3 2 3h11.991zM14 2H2C0 2 0 4 0 4v6c0 2 2 2 2 2h12c2 0 2-2 2-2V4c0-2-2-2-2-2z"/>
          </svg>
    </label>

    <div class="SetTopBox-Btns">
        <div class="arrow-keys-box">
            <div class="arrowBtn arrow-ok" type="nec" code="543023135" onclick="sendCMDByObj(this)"></div>
            <div class="arrowBtn arrow-up" type="nec" code="543014975" onclick="sendCMDByObj(this)">
                <svg xmlns="http://www.w3.org/2000/svg" class="arrow-icon" viewBox="0 0 16 16">
                    <path fill-rule="evenodd" d="M7.646 4.646a.5.5 0 0 1 .708 0l6 6a.5.5 0 0 1-.708.708L8 5.707l-5.646 5.647a.5.5 0 0 1-.708-.708l6-6z"/>
                  </svg>
            </div>
            <div class="arrowBtn arrow-right" type="nec" code="542988455" onclick="sendCMDByObj(this)">
                <svg xmlns="http://www.w3.org/2000/svg" class="arrow-icon" viewBox="0 0 16 16">
                    <path fill-rule="evenodd" d="M7.646 4.646a.5.5 0 0 1 .708 0l6 6a.5.5 0 0 1-.708.708L8 5.707l-5.646 5.647a.5.5 0 0 1-.708-.708l6-6z"/>
                  </svg>
            </div>
            <div class="arrowBtn arrow-left" type="nec" code="542994575" onclick="sendCMDByObj(this)">
                <svg xmlns="http://www.w3.org/2000/svg" class="arrow-icon" viewBox="0 0 16 16">
                    <path fill-rule="evenodd" d="M7.646 4.646a.5.5 0 0 1 .708 0l6 6a.5.5 0 0 1-.708.708L8 5.707l-5.646 5.647a.5.5 0 0 1-.708-.708l6-6z"/>
                  </svg>
            </div>
            <div class="arrowBtn arrow-down" type="nec" code="542982335" onclick="sendCMDByObj(this)">
                <svg xmlns="http://www.w3.org/2000/svg" class="arrow-icon" viewBox="0 0 16 16">
                    <path fill-rule="evenodd" d="M7.646 4.646a.5.5 0 0 1 .708 0l6 6a.5.5 0 0 1-.708.708L8 5.707l-5.646 5.647a.5.5 0 0 1-.708-.708l6-6z"/>
                  </svg>
            </div>
        </div>

        <div class="slide-key-box">
            <div class="volume-key slide-key">
                <div class="left-slide-key" type="sony" code="3216" onclick="sendCMDByObj(this)">
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-volume-down" viewBox="0 0 16 16">
                        <path d="M9 4a.5.5 0 0 0-.812-.39L5.825 5.5H3.5A.5.5 0 0 0 3 6v4a.5.5 0 0 0 .5.5h2.325l2.363 1.89A.5.5 0 0 0 9 12V4zM6.312 6.39 8 5.04v5.92L6.312 9.61A.5.5 0 0 0 6 9.5H4v-3h2a.5.5 0 0 0 .312-.11zM12.025 8a4.486 4.486 0 0 1-1.318 3.182L10 10.475A3.489 3.489 0 0 0 11.025 8 3.49 3.49 0 0 0 10 5.525l.707-.707A4.486 4.486 0 0 1 12.025 8z"/>
                      </svg>
                </div>
                <div class="right-slide-key" type="sony" code="1168" onclick="sendCMDByObj(this)">
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-volume-up" viewBox="0 0 16 16">
                        <path d="M11.536 14.01A8.473 8.473 0 0 0 14.026 8a8.473 8.473 0 0 0-2.49-6.01l-.708.707A7.476 7.476 0 0 1 13.025 8c0 2.071-.84 3.946-2.197 5.303l.708.707z"/>
                        <path d="M10.121 12.596A6.48 6.48 0 0 0 12.025 8a6.48 6.48 0 0 0-1.904-4.596l-.707.707A5.483 5.483 0 0 1 11.025 8a5.483 5.483 0 0 1-1.61 3.89l.706.706z"/>
                        <path d="M10.025 8a4.486 4.486 0 0 1-1.318 3.182L8 10.475A3.489 3.489 0 0 0 9.025 8c0-.966-.392-1.841-1.025-2.475l.707-.707A4.486 4.486 0 0 1 10.025 8zM7 4a.5.5 0 0 0-.812-.39L3.825 5.5H1.5A.5.5 0 0 0 1 6v4a.5.5 0 0 0 .5.5h2.325l2.363 1.89A.5.5 0 0 0 7 12V4zM4.312 6.39 6 5.04v5.92L4.312 9.61A.5.5 0 0 0 4 9.5H2v-3h2a.5.5 0 0 0 .312-.11z"/>
                      </svg>
                </div>
            </div>

            <div class="channel-key slide-key">
                <div class="left-slide-key" type="nec" code="542982335" onclick="sendCMDByObj(this)">
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-skip-start" viewBox="0 0 16 16">
                        <path d="M4 4a.5.5 0 0 1 1 0v3.248l6.267-3.636c.52-.302 1.233.043 1.233.696v7.384c0 .653-.713.998-1.233.696L5 8.752V12a.5.5 0 0 1-1 0V4zm7.5.633L5.696 8l5.804 3.367V4.633z"/>
                      </svg>
                </div>
                <div class="right-slide-key" type="nec" code="543014975" onclick="sendCMDByObj(this)">
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-skip-end" viewBox="0 0 16 16">
                        <path d="M12.5 4a.5.5 0 0 0-1 0v3.248L5.233 3.612C4.713 3.31 4 3.655 4 4.308v7.384c0 .653.713.998 1.233.696L11.5 8.752V12a.5.5 0 0 0 1 0V4zM5 4.633 10.804 8 5 11.367V4.633z"/>
                      </svg>
                </div>
            </div>
        </div>
    
        <div class="func-keys-box">
            <div class="func-btns" type="sony" code="656" onclick="sendCMDByObj(this)">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-volume-mute" viewBox="0 0 16 16">
                    <path d="M6.717 3.55A.5.5 0 0 1 7 4v8a.5.5 0 0 1-.812.39L3.825 10.5H1.5A.5.5 0 0 1 1 10V6a.5.5 0 0 1 .5-.5h2.325l2.363-1.89a.5.5 0 0 1 .529-.06zM6 5.04 4.312 6.39A.5.5 0 0 1 4 6.5H2v3h2a.5.5 0 0 1 .312.11L6 10.96V5.04zm7.854.606a.5.5 0 0 1 0 .708L12.207 8l1.647 1.646a.5.5 0 0 1-.708.708L11.5 8.707l-1.646 1.647a.5.5 0 0 1-.708-.708L10.793 8 9.146 6.354a.5.5 0 1 1 .708-.708L11.5 7.293l1.646-1.647a.5.5 0 0 1 .708 0z"/>
                  </svg>
                <div class="func-fullname">靜音</div>
            </div>

            <div class="func-btns" type="sony" code="2640" onclick="sendCMDByObj(this)">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-cast" viewBox="0 0 16 16">
                    <path d="m7.646 9.354-3.792 3.792a.5.5 0 0 0 .353.854h7.586a.5.5 0 0 0 .354-.854L8.354 9.354a.5.5 0 0 0-.708 0z"/>
                    <path d="M11.414 11H14.5a.5.5 0 0 0 .5-.5v-7a.5.5 0 0 0-.5-.5h-13a.5.5 0 0 0-.5.5v7a.5.5 0 0 0 .5.5h3.086l-1 1H1.5A1.5 1.5 0 0 1 0 10.5v-7A1.5 1.5 0 0 1 1.5 2h13A1.5 1.5 0 0 1 16 3.5v7a1.5 1.5 0 0 1-1.5 1.5h-2.086l-1-1z"/>
                  </svg>
                <div class="func-fullname">輸入源</div>
            </div>

            <div class="func-btns" type="nec" code="542967035" onclick="sendCMDByObj(this)">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-box-arrow-left" viewBox="0 0 16 16">
                    <path fill-rule="evenodd" d="M6 12.5a.5.5 0 0 0 .5.5h8a.5.5 0 0 0 .5-.5v-9a.5.5 0 0 0-.5-.5h-8a.5.5 0 0 0-.5.5v2a.5.5 0 0 1-1 0v-2A1.5 1.5 0 0 1 6.5 2h8A1.5 1.5 0 0 1 16 3.5v9a1.5 1.5 0 0 1-1.5 1.5h-8A1.5 1.5 0 0 1 5 12.5v-2a.5.5 0 0 1 1 0v2z"/>
                    <path fill-rule="evenodd" d="M.146 8.354a.5.5 0 0 1 0-.708l3-3a.5.5 0 1 1 .708.708L1.707 7.5H10.5a.5.5 0 0 1 0 1H1.707l2.147 2.146a.5.5 0 0 1-.708.708l-3-3z"/>
                  </svg>
                <div class="func-fullname">退出</div>
            </div>

            <div class="func-btns" type="nec" code="542972135" onclick="sendCMDByObj(this)">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-list-columns" viewBox="0 0 16 16">
                    <path fill-rule="evenodd" d="M0 .5A.5.5 0 0 1 .5 0h9a.5.5 0 0 1 0 1h-9A.5.5 0 0 1 0 .5Zm13 0a.5.5 0 0 1 .5-.5h2a.5.5 0 0 1 0 1h-2a.5.5 0 0 1-.5-.5Zm-13 2A.5.5 0 0 1 .5 2h8a.5.5 0 0 1 0 1h-8a.5.5 0 0 1-.5-.5Zm13 0a.5.5 0 0 1 .5-.5h2a.5.5 0 0 1 0 1h-2a.5.5 0 0 1-.5-.5Zm-13 2A.5.5 0 0 1 .5 4h10a.5.5 0 0 1 0 1H.5a.5.5 0 0 1-.5-.5Zm13 0a.5.5 0 0 1 .5-.5h2a.5.5 0 0 1 0 1h-2a.5.5 0 0 1-.5-.5Zm-13 2A.5.5 0 0 1 .5 6h7a.5.5 0 0 1 0 1h-7a.5.5 0 0 1-.5-.5Zm13 0a.5.5 0 0 1 .5-.5h2a.5.5 0 0 1 0 1h-2a.5.5 0 0 1-.5-.5Zm-13 2A.5.5 0 0 1 .5 8h7a.5.5 0 0 1 0 1h-7a.5.5 0 0 1-.5-.5Zm13 0a.5.5 0 0 1 .5-.5h2a.5.5 0 0 1 0 1h-2a.5.5 0 0 1-.5-.5Zm-13 2a.5.5 0 0 1 .5-.5h9a.5.5 0 0 1 0 1h-9a.5.5 0 0 1-.5-.5Zm13 0a.5.5 0 0 1 .5-.5h2a.5.5 0 0 1 0 1h-2a.5.5 0 0 1-.5-.5Zm-13 2a.5.5 0 0 1 .5-.5h5a.5.5 0 0 1 0 1h-5a.5.5 0 0 1-.5-.5Zm13 0a.5.5 0 0 1 .5-.5h2a.5.5 0 0 1 0 1h-2a.5.5 0 0 1-.5-.5Zm-13 2a.5.5 0 0 1 .5-.5h10a.5.5 0 0 1 0 1H.5a.5.5 0 0 1-.5-.5Zm13 0a.5.5 0 0 1 .5-.5h2a.5.5 0 0 1 0 1h-2a.5.5 0 0 1-.5-.5Z"/>
                  </svg>
                <div class="func-fullname">節目詳細</div>
            </div>

            <div class="func-btns" type="nec" code="542983355" onclick="sendCMDByObj(this)">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-gear" viewBox="0 0 16 16">
                    <path d="M8 4.754a3.246 3.246 0 1 0 0 6.492 3.246 3.246 0 0 0 0-6.492zM5.754 8a2.246 2.246 0 1 1 4.492 0 2.246 2.246 0 0 1-4.492 0z"/>
                    <path d="M9.796 1.343c-.527-1.79-3.065-1.79-3.592 0l-.094.319a.873.873 0 0 1-1.255.52l-.292-.16c-1.64-.892-3.433.902-2.54 2.541l.159.292a.873.873 0 0 1-.52 1.255l-.319.094c-1.79.527-1.79 3.065 0 3.592l.319.094a.873.873 0 0 1 .52 1.255l-.16.292c-.892 1.64.901 3.434 2.541 2.54l.292-.159a.873.873 0 0 1 1.255.52l.094.319c.527 1.79 3.065 1.79 3.592 0l.094-.319a.873.873 0 0 1 1.255-.52l.292.16c1.64.893 3.434-.902 2.54-2.541l-.159-.292a.873.873 0 0 1 .52-1.255l.319-.094c1.79-.527 1.79-3.065 0-3.592l-.319-.094a.873.873 0 0 1-.52-1.255l.16-.292c.893-1.64-.902-3.433-2.541-2.54l-.292.159a.873.873 0 0 1-1.255-.52l-.094-.319zm-2.633.283c.246-.835 1.428-.835 1.674 0l.094.319a1.873 1.873 0 0 0 2.693 1.115l.291-.16c.764-.415 1.6.42 1.184 1.185l-.159.292a1.873 1.873 0 0 0 1.116 2.692l.318.094c.835.246.835 1.428 0 1.674l-.319.094a1.873 1.873 0 0 0-1.115 2.693l.16.291c.415.764-.42 1.6-1.185 1.184l-.291-.159a1.873 1.873 0 0 0-2.693 1.116l-.094.318c-.246.835-1.428.835-1.674 0l-.094-.319a1.873 1.873 0 0 0-2.692-1.115l-.292.16c-.764.415-1.6-.42-1.184-1.185l.159-.291A1.873 1.873 0 0 0 1.945 8.93l-.319-.094c-.835-.246-.835-1.428 0-1.674l.319-.094A1.873 1.873 0 0 0 3.06 4.377l-.16-.292c-.415-.764.42-1.6 1.185-1.184l.292.159a1.873 1.873 0 0 0 2.692-1.115l.094-.319z"/>
                  </svg>
                <div class="func-fullname">設置菜單</div>
            </div>

            <div class="func-btns" type="sony" code="2704" onclick="sendCMDByObj(this)">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-power" viewBox="0 0 16 16">
                    <path d="M7.5 1v7h1V1h-1z"/>
                    <path d="M3 8.812a4.999 4.999 0 0 1 2.578-4.375l-.485-.874A6 6 0 1 0 11 3.616l-.501.865A5 5 0 1 1 3 8.812z"/>
                  </svg>
                <div class="func-fullname">電源</div>
            </div>

            <div class="func-btns" type="nec" code="543017015" onclick="sendCMDByObj(this)">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-ear" viewBox="0 0 16 16">
                    <path d="M8.5 1A4.5 4.5 0 0 0 4 5.5v7.047a2.453 2.453 0 0 0 4.75.861l.512-1.363a5.553 5.553 0 0 1 .816-1.46l2.008-2.581A4.34 4.34 0 0 0 8.66 1H8.5ZM3 5.5A5.5 5.5 0 0 1 8.5 0h.16a5.34 5.34 0 0 1 4.215 8.618l-2.008 2.581a4.555 4.555 0 0 0-.67 1.197l-.51 1.363A3.453 3.453 0 0 1 3 12.547V5.5ZM8.5 4A1.5 1.5 0 0 0 7 5.5v2.695c.112-.06.223-.123.332-.192.327-.208.577-.44.72-.727a.5.5 0 1 1 .895.448c-.256.513-.673.865-1.079 1.123A8.538 8.538 0 0 1 7 9.313V11.5a.5.5 0 0 1-1 0v-6a2.5 2.5 0 0 1 5 0V6a.5.5 0 0 1-1 0v-.5A1.5 1.5 0 0 0 8.5 4Z"/>
                  </svg>
                <div class="func-fullname">聲道</div>
            </div>

            <!-- <div class="func-btns">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-123" viewBox="0 0 16 16">
                    <path d="M2.873 11.297V4.142H1.699L0 5.379v1.137l1.64-1.18h.06v5.961h1.174Zm3.213-5.09v-.063c0-.618.44-1.169 1.196-1.169.676 0 1.174.44 1.174 1.106 0 .624-.42 1.101-.807 1.526L4.99 10.553v.744h4.78v-.99H6.643v-.069L8.41 8.252c.65-.724 1.237-1.332 1.237-2.27C9.646 4.849 8.723 4 7.308 4c-1.573 0-2.36 1.064-2.36 2.15v.057h1.138Zm6.559 1.883h.786c.823 0 1.374.481 1.379 1.179.01.707-.55 1.216-1.421 1.21-.77-.005-1.326-.419-1.379-.953h-1.095c.042 1.053.938 1.918 2.464 1.918 1.478 0 2.642-.839 2.62-2.144-.02-1.143-.922-1.651-1.551-1.714v-.063c.535-.09 1.347-.66 1.326-1.678-.026-1.053-.933-1.855-2.359-1.845-1.5.005-2.317.88-2.348 1.898h1.116c.032-.498.498-.944 1.206-.944.703 0 1.206.435 1.206 1.07.005.64-.504 1.106-1.2 1.106h-.75v.96Z"/>
                  </svg>
                <div class="func-fullname">數字鍵</div>
            </div> -->
        </div>
    </div>

    <div class="TV-Btns">
        <div class="arrow-keys-box">
            <div class="arrowBtn arrow-ok" type="sony" code="2672" onclick="sendCMDByObj(this)"></div>
            <div class="arrowBtn arrow-up" type="sony" code="752" onclick="sendCMDByObj(this)">
                <svg xmlns="http://www.w3.org/2000/svg" class="arrow-icon" viewBox="0 0 16 16">
                    <path fill-rule="evenodd" d="M7.646 4.646a.5.5 0 0 1 .708 0l6 6a.5.5 0 0 1-.708.708L8 5.707l-5.646 5.647a.5.5 0 0 1-.708-.708l6-6z"/>
                  </svg>
            </div>
            <div class="arrowBtn arrow-right" type="sony" code="3280" onclick="sendCMDByObj(this)">
                <svg xmlns="http://www.w3.org/2000/svg" class="arrow-icon" viewBox="0 0 16 16">
                    <path fill-rule="evenodd" d="M7.646 4.646a.5.5 0 0 1 .708 0l6 6a.5.5 0 0 1-.708.708L8 5.707l-5.646 5.647a.5.5 0 0 1-.708-.708l6-6z"/>
                  </svg>
            </div>
            <div class="arrowBtn arrow-left" type="sony" code="720" onclick="sendCMDByObj(this)">
                <svg xmlns="http://www.w3.org/2000/svg" class="arrow-icon" viewBox="0 0 16 16">
                    <path fill-rule="evenodd" d="M7.646 4.646a.5.5 0 0 1 .708 0l6 6a.5.5 0 0 1-.708.708L8 5.707l-5.646 5.647a.5.5 0 0 1-.708-.708l6-6z"/>
                  </svg>
            </div>
            <div class="arrowBtn arrow-down" type="sony" code="2800" onclick="sendCMDByObj(this)">
                <svg xmlns="http://www.w3.org/2000/svg" class="arrow-icon" viewBox="0 0 16 16">
                    <path fill-rule="evenodd" d="M7.646 4.646a.5.5 0 0 1 .708 0l6 6a.5.5 0 0 1-.708.708L8 5.707l-5.646 5.647a.5.5 0 0 1-.708-.708l6-6z"/>
                  </svg>
            </div>
        </div>

        <div class="slide-key-box">
            <div class="volume-key slide-key">
                <div class="left-slide-key" type="nec" code="542994575" onclick="sendCMDByObj(this)">
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-volume-down" viewBox="0 0 16 16">
                        <path d="M9 4a.5.5 0 0 0-.812-.39L5.825 5.5H3.5A.5.5 0 0 0 3 6v4a.5.5 0 0 0 .5.5h2.325l2.363 1.89A.5.5 0 0 0 9 12V4zM6.312 6.39 8 5.04v5.92L6.312 9.61A.5.5 0 0 0 6 9.5H4v-3h2a.5.5 0 0 0 .312-.11zM12.025 8a4.486 4.486 0 0 1-1.318 3.182L10 10.475A3.489 3.489 0 0 0 11.025 8 3.49 3.49 0 0 0 10 5.525l.707-.707A4.486 4.486 0 0 1 12.025 8z"/>
                      </svg>
                </div>
                <div class="right-slide-key" type="nec" code="542988455" onclick="sendCMDByObj(this)">
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-volume-up" viewBox="0 0 16 16">
                        <path d="M11.536 14.01A8.473 8.473 0 0 0 14.026 8a8.473 8.473 0 0 0-2.49-6.01l-.708.707A7.476 7.476 0 0 1 13.025 8c0 2.071-.84 3.946-2.197 5.303l.708.707z"/>
                        <path d="M10.121 12.596A6.48 6.48 0 0 0 12.025 8a6.48 6.48 0 0 0-1.904-4.596l-.707.707A5.483 5.483 0 0 1 11.025 8a5.483 5.483 0 0 1-1.61 3.89l.706.706z"/>
                        <path d="M10.025 8a4.486 4.486 0 0 1-1.318 3.182L8 10.475A3.489 3.489 0 0 0 9.025 8c0-.966-.392-1.841-1.025-2.475l.707-.707A4.486 4.486 0 0 1 10.025 8zM7 4a.5.5 0 0 0-.812-.39L3.825 5.5H1.5A.5.5 0 0 0 1 6v4a.5.5 0 0 0 .5.5h2.325l2.363 1.89A.5.5 0 0 0 7 12V4zM4.312 6.39 6 5.04v5.92L4.312 9.61A.5.5 0 0 0 4 9.5H2v-3h2a.5.5 0 0 0 .312-.11z"/>
                      </svg>
                </div>
            </div>

        </div>
    
        <div class="func-keys-box">
            <div class="func-btns" type="sony" code="1006" bits="15" onclick="sendCMDByObj(this)">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-camera-reels" viewBox="0 0 16 16">
                    <path d="M6 3a3 3 0 1 1-6 0 3 3 0 0 1 6 0zM1 3a2 2 0 1 0 4 0 2 2 0 0 0-4 0z"/>
                    <path d="M9 6h.5a2 2 0 0 1 1.983 1.738l3.11-1.382A1 1 0 0 1 16 7.269v7.462a1 1 0 0 1-1.406.913l-3.111-1.382A2 2 0 0 1 9.5 16H2a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h7zm6 8.73V7.27l-3.5 1.555v4.35l3.5 1.556zM1 8v6a1 1 0 0 0 1 1h7.5a1 1 0 0 0 1-1V8a1 1 0 0 0-1-1H2a1 1 0 0 0-1 1z"/>
                    <path d="M9 6a3 3 0 1 0 0-6 3 3 0 0 0 0 6zM7 3a2 2 0 1 1 4 0 2 2 0 0 1-4 0z"/>
                  </svg>
                  <div class="func-fullname">劇院模式</div>
            </div>

            <div class="func-btns" type="sony" code="25321" bits="15" onclick="sendCMDByObj(this)">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-box-arrow-left" viewBox="0 0 16 16">
                    <path fill-rule="evenodd" d="M6 12.5a.5.5 0 0 0 .5.5h8a.5.5 0 0 0 .5-.5v-9a.5.5 0 0 0-.5-.5h-8a.5.5 0 0 0-.5.5v2a.5.5 0 0 1-1 0v-2A1.5 1.5 0 0 1 6.5 2h8A1.5 1.5 0 0 1 16 3.5v9a1.5 1.5 0 0 1-1.5 1.5h-8A1.5 1.5 0 0 1 5 12.5v-2a.5.5 0 0 1 1 0v2z"/>
                    <path fill-rule="evenodd" d="M.146 8.354a.5.5 0 0 1 0-.708l3-3a.5.5 0 1 1 .708.708L1.707 7.5H10.5a.5.5 0 0 1 0 1H1.707l2.147 2.146a.5.5 0 0 1-.708.708l-3-3z"/>
                  </svg>
                <div class="func-fullname">退出</div>
            </div>

            <div class="func-btns" type="sony" code="112" onclick="sendCMDByObj(this)">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-gear" viewBox="0 0 16 16">
                    <path d="M8 4.754a3.246 3.246 0 1 0 0 6.492 3.246 3.246 0 0 0 0-6.492zM5.754 8a2.246 2.246 0 1 1 4.492 0 2.246 2.246 0 0 1-4.492 0z"/>
                    <path d="M9.796 1.343c-.527-1.79-3.065-1.79-3.592 0l-.094.319a.873.873 0 0 1-1.255.52l-.292-.16c-1.64-.892-3.433.902-2.54 2.541l.159.292a.873.873 0 0 1-.52 1.255l-.319.094c-1.79.527-1.79 3.065 0 3.592l.319.094a.873.873 0 0 1 .52 1.255l-.16.292c-.892 1.64.901 3.434 2.541 2.54l.292-.159a.873.873 0 0 1 1.255.52l.094.319c.527 1.79 3.065 1.79 3.592 0l.094-.319a.873.873 0 0 1 1.255-.52l.292.16c1.64.893 3.434-.902 2.54-2.541l-.159-.292a.873.873 0 0 1 .52-1.255l.319-.094c1.79-.527 1.79-3.065 0-3.592l-.319-.094a.873.873 0 0 1-.52-1.255l.16-.292c.893-1.64-.902-3.433-2.541-2.54l-.292.159a.873.873 0 0 1-1.255-.52l-.094-.319zm-2.633.283c.246-.835 1.428-.835 1.674 0l.094.319a1.873 1.873 0 0 0 2.693 1.115l.291-.16c.764-.415 1.6.42 1.184 1.185l-.159.292a1.873 1.873 0 0 0 1.116 2.692l.318.094c.835.246.835 1.428 0 1.674l-.319.094a1.873 1.873 0 0 0-1.115 2.693l.16.291c.415.764-.42 1.6-1.185 1.184l-.291-.159a1.873 1.873 0 0 0-2.693 1.116l-.094.318c-.246.835-1.428.835-1.674 0l-.094-.319a1.873 1.873 0 0 0-2.692-1.115l-.292.16c-.764.415-1.6-.42-1.184-1.185l.159-.291A1.873 1.873 0 0 0 1.945 8.93l-.319-.094c-.835-.246-.835-1.428 0-1.674l.319-.094A1.873 1.873 0 0 0 3.06 4.377l-.16-.292c-.415-.764.42-1.6 1.185-1.184l.292.159a1.873 1.873 0 0 0 2.692-1.115l.094-.319z"/>
                  </svg>
                <div class="func-fullname">設置菜單</div>
            </div>

            <div class="func-btns" type="nec" code="542980295" onclick="sendCMDByObj(this)">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-power" viewBox="0 0 16 16">
                    <path d="M7.5 1v7h1V1h-1z"/>
                    <path d="M3 8.812a4.999 4.999 0 0 1 2.578-4.375l-.485-.874A6 6 0 1 0 11 3.616l-.501.865A5 5 0 1 1 3 8.812z"/>
                  </svg>
                <div class="func-fullname">電源</div>
            </div>

            <!-- <div class="func-btns">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-123" viewBox="0 0 16 16">
                    <path d="M2.873 11.297V4.142H1.699L0 5.379v1.137l1.64-1.18h.06v5.961h1.174Zm3.213-5.09v-.063c0-.618.44-1.169 1.196-1.169.676 0 1.174.44 1.174 1.106 0 .624-.42 1.101-.807 1.526L4.99 10.553v.744h4.78v-.99H6.643v-.069L8.41 8.252c.65-.724 1.237-1.332 1.237-2.27C9.646 4.849 8.723 4 7.308 4c-1.573 0-2.36 1.064-2.36 2.15v.057h1.138Zm6.559 1.883h.786c.823 0 1.374.481 1.379 1.179.01.707-.55 1.216-1.421 1.21-.77-.005-1.326-.419-1.379-.953h-1.095c.042 1.053.938 1.918 2.464 1.918 1.478 0 2.642-.839 2.62-2.144-.02-1.143-.922-1.651-1.551-1.714v-.063c.535-.09 1.347-.66 1.326-1.678-.026-1.053-.933-1.855-2.359-1.845-1.5.005-2.317.88-2.348 1.898h1.116c.032-.498.498-.944 1.206-.944.703 0 1.206.435 1.206 1.07.005.64-.504 1.106-1.2 1.106h-.75v.96Z"/>
                  </svg>
                <div class="func-fullname">數字鍵</div>
            </div> -->
        </div>
    </div>

    <div class="footer"></div>
`
 

const headHTML = `
    <title>電視遙控器</title>
    <link rel="apple-touch-icon" href="https://ikwbb.github.io/TV_Remote/favicon" />
    <meta name="apple-mobile-web-app-capable" content="yes" />
`

window.addEventListener('load', ()=>{
    document.head.innerHTML += headHTML
    document.body.innerHTML = bodyHTML 

    const stylesheet = document.createElement("style")
    stylesheet.innerHTML = css
    document.head.appendChild(stylesheet)

    init()
})


function init(){
    const channelSelector = document.querySelector(".channel-selector")
    const channelBar = document.querySelector(".channel-activate-bar-box")
    const chevronUp = document.querySelector(".bi-chevron-up")
    const channelSelectorCloser = document.querySelector(".channel-selector-closer")

    function toggleHideList() {
        channelSelector.classList.toggle("hide-list");
        chevronUp.classList.toggle("rotate180");
        channelSelectorCloser.classList.toggle("closer-hidden");
    }

    channelBar.addEventListener('click', ()=>{
        toggleHideList();
    })

    channelSelectorCloser.addEventListener('click', ()=>{
        toggleHideList();
    })

}

function sendCMDByObj(obj) {
    const type = obj.getAttribute('type');
    const code = obj.getAttribute('code');
    const bits = obj.getAttribute('bits')
    if(bits == void 0) sendCMD(code, type);
    else sendCMD(code, type, bits);
    console.log(`type: ${type}, code: ${code}, bits: ${bits}`);
}

function sendCMD(code, type, bits=void 0){
    const controller = new AbortController();
    const signal = controller.signal;
    var url = `http://192.168.3.29/cmd?type=${type}&code=${code}`;
    if(bits) url += `&bits=${bits}`
    fetch(url, {signal}).then(res => {
        controller.abort();
    })
}

function setFavicons(favImg){
    let headTitle = document.querySelector('head');
    let setFavicon = document.createElement('link');
    setFavicon.setAttribute('rel','shortcut icon');
    setFavicon.setAttribute('href',favImg);
    headTitle.appendChild(setFavicon);
}