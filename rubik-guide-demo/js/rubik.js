...
Cube.prototype.setRotation = function(rotation, hasAnimation) {
	...
	this._update(hasAnimation ? 300 : 1);
}
...
Rubik.prototype.init = function() {
	...
	this._rotation = Quaternion.fromRotation([1, 0, 0], -20).multiply(Quaternion.fromRotation([0, 1, 0], -30));
	...
	for (var z=0;z<Rubik.SIZE;z++) {
		for (var y=0;y<Rubik.SIZE;y++) {
			for (var x=0;x<Rubik.SIZE;x++) {
				...
				if (z == 2) { 
					cube.setFace(Face.BACK, "green"); 
					cube.setFace(Face.FRONT, inside); 
				}
				
				if (x == 0) { 
					cube.setFace(Face.LEFT, "orange"); 
				...
				if (x == 2) { 
					cube.setFace(Face.RIGHT, "red"); 
					cube.setFace(Face.LEFT, inside); 
				}
				
				if (y == 0) { 
			 		cube.setFace(Face.TOP, "yellow"); 
				...
				if (y == 2) { 
					cube.setFace(Face.BOTTOM, "white");  
					cube.setFace(Face.TOP, inside); 
				}
...
Rubik.prototype._execAlgo = function(algoArr, speed=1000) {
	const that = this;
	...
	function recurExec(recurArr) {
		// for (var turn of algoArr) { this._execTurn(turn); }
		if (recurArr.length == 0) return;
		that._execTurn(recurArr[0]);
		setTimeout(() => { recurExec(recurArr.slice(1))}, speed);
	}

	recurExec(getAtomicArr(algoArr));
